...
check "gemini full id from line 1" "$ cd $proj && gemini --resume ffff6666-0000-0000-0000-000000000000" "$(bash "$ALS" -a -t gemini -l | sed -n 2p | sed 's/^ *//')"
check "cline resume" "$ cd $proj && cline -i --id 1758500000000_ab1cd" "$(bash "$ALS" -a -t cline -l | sed -n 2p | sed 's/^ *//')"
check "qwen resume" "$ cd $proj && qwen --resume eeee5555-0000-0000-0000-000000000000" "$(bash "$ALS" -a -t qwen -l | sed -n 2p | sed 's/^ *//')"
check "-l in the session's own directory has no cd" "$ codex resume bbbb2222-0000-0000-0000-000000000000" "$(cd "$proj" && bash "$ALS" -t codex -l | sed -n 2p | sed 's/^ *//')"
check "openclaw -l never has a cd" "$ openclaw tui --session agent:main:main" "$(bash "$ALS" -a -t openclaw -l 2>/dev/null | sed -n 2p | sed 's/^ *//')"
...
if [ $have_sqlite = 1 ]; then
  ...
  check "opencode resume" "$ cd $proj && opencode --session ses_gggg7777aaaa" "$(bash "$ALS" -a -t opencode -l | sed -n 2p | sed 's/^ *//')"
  check "goose resume" "$ cd $proj && goose session --resume --session-id 20260922_7" "$(bash "$ALS" -a -t goose -l | sed -n 2p | sed 's/^ *//')"
  check "muse resume" "$ cd $proj && muse resume hhhh8888-0000-0000-0000-000000000000" "$(bash "$ALS" -a -t muse -l | sed -n 2p | sed 's/^ *//')"
...
check "-l for pi uses the file" "$ cd $tmp/elsewhere && pi --session $tmp/.pi/agent/sessions/x/2026-09-22T09-00-00-000Z_cccc3333-0000-0000-0000-000000000000.jsonl" "$(bash "$ALS" -a -t pi -l | sed -n 2p | sed 's/^ *//')"
